...
public class IfcHashMapBuilder {
	...
	public List<HashMap<AbstractRule, ObjectToValue>> getHashMaps()
		...
		List<HashMap<AttributeRule, ObjectToValue>> hMs = buildHashMaps(
				ifcObject, attributeRules, initialHMs);
...
/*			if (isConstraint(attributeRule,concept)){
				if (attributeRule.getCardinality().equals("_asSchema")){
...
/*	private boolean isConstraint(AttributeRule attributeRule,Concept concept){
		boolean bool=false;
		if (!attributeRule.getCardinality().equals("_asSchema")){
			bool=true;
		}
		else if (isReferencedBy(attributeRule,concept)){
			bool=true;
		}
		return bool;
	}
	
	private boolean isReferencedBy(AttributeRule attributeRule,Concept concept){
		boolean bool=false;
		ListTemplateRule tr=concept.getRules()
		return bool;
	}*/
